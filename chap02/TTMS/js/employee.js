...
function add() {
	...
	var sss1 = document.getElementById('recipient-name6');
	...
	var info2 = document.createTextNode(sss1.value);
	td6.appendChild(info2);
...
function name3() {
	...
	if (row.value == "女" || row.value == "男") {
		...
		document.getElementById("name3_tip").innerHTML = "请输入 男 或 女 ！";
		row.focus();
		return false;
	}
}
...
function name5() {
	var info = document.getElementById('recipient-name5');
	if (!/^1[3-9][0-9]{9}$/.test(info.value)) {
		document.getElementById('name5_img').innerHTML = "<img src='images/error.png' />";
		document.getElementById("name5_tip").innerHTML = "请输入正确的手机号码！";
		info.focus();
		return false;
	} else {
